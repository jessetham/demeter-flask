...
def create_reading(sensor_id):
    ...
    response.headers["Location"] = url_for(
        "api.get_reading", reading_id=reading.id, sensor_id=sensor.id
    )
    return response
...
def get_readings(sensor_id):
    ...
    sensor = Sensor.query.get_or_404(sensor_id)
    data = Sensor.to_collection_dict(
        sensor.readings, page, per_page, "api.get_readings", sensor_id=sensor_id
    )
    return jsonify(data)
