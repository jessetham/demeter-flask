...
class ReadingsAPICase(BaseAPICase):
    def test_create(self):
        ...
        res = self.client.post(
            "api/sensors/42/readings", json={"data": 404, "category": "temperature"}
        )
        ...
        res = self.client.post(
            "api/sensors/{}/readings".format(sensor["id"]), json={"category": "failure"}
        )
        self.assertEqual(res.status_code, 400, res.get_json())

        # Test adding a reading with no category parameter
        res = self.client.post(
            "api/sensors/{}/readings".format(sensor["id"]), json={"data": 400}
        )
        self.assertEqual(res.status_code, 400, res.get_json())
        ...
        for category in sensor["categories"]:
            ...
            res = self.client.post(
                "/api/sensors/{}/readings".format(sensor["id"]), json=reading
            )
    ...
    def test_get_single(self):
        ...
        readings = [
            reading["id"]
            for reading in utl.READINGS
            if reading["sensor"] == sensor["id"]
        ]
        # Test getting readings that do exist
        for reading in readings:
            res = self.client.get(
                "api/sensors/{}/readings/{}".format(sensor["id"], reading)
            )
